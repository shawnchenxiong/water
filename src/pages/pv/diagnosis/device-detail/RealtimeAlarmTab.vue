...
const selectedAlarms = ref<AlarmInfo[]>([])
...
const handleBatchConfirm = async () => {
  ...
  try {
    await ElMessageBox.confirm(
      `确定要批量确认选中的 ${selectedAlarms.value.length} 条告警吗？`,
      '批量确认',
      {
    ...
    // 这里应该调用批量确认API
    ElMessage.success(`已成功确认 ${selectedAlarms.value.length} 条告警`)
  ...
  .table-section {
    ...
    :deep(.el-table) {
      ...
      .el-table__row {
        ...
        &.critical-row {
          background: rgba(231, 76, 60, 0.3) !important;  // 严重告警红色背景
        }
      }
