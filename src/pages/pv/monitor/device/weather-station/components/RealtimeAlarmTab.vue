<template>
  <div v-loading="loading" class="realtime-alarm-tab">
    ...
    <div class="table-wrapper">
      ...
      >
        ...
        <el-table-column prop="status" label="确认状态" width="100">
          <template #default="{ row }">
        ...
        <el-table-column label="操作" width="120" fixed="right">
          <template #default="{ row }">
            <el-button link type="primary" :size="isMobile ? 'small' : 'small'" @click="handleConfirm(row)">确认</el-button>
            <el-button link type="danger" :size="isMobile ? 'small' : 'small'" @click="handleClear(row)">消除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
import { fetchRealAlarmList } from '@/api/device/weatherStation';
...
const loadData = async () => {
  loading.value = true;
  try {
    const result = await fetchRealAlarmList({
...
const handleBatchClear = () => {
  console.log('Batch clear alarms');
};

const handleBatchConfirm = () => {
  console.log('Batch confirm alarms');
};

const handleConfirm = (row: AlarmItem) => {
  console.log('Confirm alarm:', row);
};

const handleClear = (row: AlarmItem) => {
  console.log('Clear alarm:', row);
};
