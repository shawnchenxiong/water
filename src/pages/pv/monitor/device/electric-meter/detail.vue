<template>
  <DeviceMonitorLayout>
    <!-- 左侧电站树 -->
    <template #left>
      <StationTree device-type="0912" />
    </template>

    <!-- 右侧详情内容区 -->
    <template #right>
      <div class="electric-meter-detail-content">
        <!-- 顶部导航 -->
        <div class="detail-header" :class="{ 'mobile-header': isMobile }">
          ...
          <div v-if="!isMobile" class="breadcrumb-nav">
            <el-breadcrumb separator=">">
              ...
              <el-breadcrumb-item>电能表</el-breadcrumb-item>
              <el-breadcrumb-item>
                <el-select 
                  v-model="currentDeviceId" 
                  @change="handleDeviceChange"
...
import { getSiblingDevices } from '@/api/device/electricMeterDetail';
...
// 返回
const handleBack = () => {
  router.push('/pv/monitor/device/electric-meter');
};

// 设备切换
const handleDeviceChange = (deviceId: string) => {
  router.replace(`/pv/monitor/device/electric-meter/${deviceId}`);
};

const handlePrevDevice = () => {
  if (hasPrev.value) {
    const prevDevice = deviceList.value[currentIndex.value - 1];
    router.replace(`/pv/monitor/device/electric-meter/${prevDevice.deviceId}`);
  }
};
...
const handleNextDevice = () => {
  if (hasNext.value) {
    ...
    router.replace(`/pv/monitor/device/electric-meter/${nextDevice.deviceId}`);
  }
};
