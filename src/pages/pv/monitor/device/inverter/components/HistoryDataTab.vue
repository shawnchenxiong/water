<template>
  <div class="history-data-tab">
    ...
    <div class="time-control" :class="{ 'mobile': isMobile }">
      <el-radio-group v-model="timeType" @change="handleTimeTypeChange" :size="isMobile ? 'small' : 'default'">
    ...
    <div class="param-control" :class="{ 'mobile': isMobile }">
      <div class="control-row">
        <el-button :size="isMobile ? 'small' : 'default'" @click="showPointSelector = true">参数选择</el-button>
        
        <div class="control-item">
          <span class="label">采样</span>
          <el-switch v-model="sampling" :size="isMobile ? 'small' : 'default'" />
        </div>
      </div>

      <div class="control-row" v-if="sampling">
        <div class="control-item">
          <span class="label">时间粒度</span>
          <el-select v-model="samplingInterval" :style="{ width: isMobile ? '100px' : '120px' }" :size="isMobile ? 'small' : 'default'">
        ...
        <div class="control-item">
          ...
          <el-select v-model="samplingType" :style="{ width: isMobile ? '100px' : '120px' }" :size="isMobile ? 'small' : 'default'">
      ...
      <div class="control-row">
        <el-button :icon="Download" :size="isMobile ? 'small' : 'default'" @click="handleExport">导出</el-button>
        <el-button :icon="Refresh" :size="isMobile ? 'small' : 'default'" @click="handleRefresh">刷新</el-button>
      </div>
    </div>
    ...
    >
      ...
      <template #footer>
        <el-button :size="isMobile ? 'small' : 'default'" @click="showPointSelector = false">取消</el-button>
        <el-button type="primary" :size="isMobile ? 'small' : 'default'" @click="handlePointConfirm">确定</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
import { getHistoryData, getPointList } from '@/api/device/inverterDetail';
import type { PointInfo } from '@/api/types/device/inverterDetail';
...
const loadChartData = async () => {
  ...
  try {
    ...
    if (timeType.value === 'custom') {
      // 自定义时间范围
      if (!customDateRange.value || !Array.isArray(customDateRange.value) || customDateRange.value.length !== 2) {
      ...
      const validDate = dayjs(selectedDate.value);
      ...
      startTime = validDate.startOf('day').toISOString();
      endTime = validDate.endOf('day').toISOString();
    }
...
const updateChart = (data: any) => {
  ...
  const option: EChartsOption = {
    ...
    legend: {
      ...
      itemHeight: isMobile.value ? 8 : 14
    },
    grid: { 
      left: isMobile.value ? '20px' : '50px', 
      right: isMobile.value ? '20px' : '50px', 
      top: isMobile.value ? '60px' : '80px', 
      bottom: isMobile.value ? '30px' : '80px',
      containLabel: true
    },
    xAxis: {
      type: 'category',
...
const handleDateChange = () => {
  if (timeType.value === 'custom') {
    ...
    if (!selectedDate.value || isNaN(new Date(selectedDate.value).getTime())) {
      ...
      selectedDate.value = new Date();
      ElMessage.warning('选择的日期无效，已重置为当前日期');
    }
  }
  loadChartData();
};
...
const handlePrevDate = () => {
  try {
    ...
    const newDate = currentDate.subtract(1, unitMap[timeType.value]);
    if (newDate.isValid()) {
      selectedDate.value = newDate.toDate();
      loadChartData();
    }
  } catch (error) {
    console.error('Error in handlePrevDate:', error);
    selectedDate.value = new Date();
  }
};
...
const handleNextDate = () => {
  try {
    ...
    const newDate = currentDate.add(1, unitMap[timeType.value]);
    if (newDate.isValid()) {
      selectedDate.value = newDate.toDate();
      loadChartData();
    }
