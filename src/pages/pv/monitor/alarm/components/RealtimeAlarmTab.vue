...
import { getDeviceRealtimeAlarms, confirmAlarm, eliminateAlarm } from '@/api/alarm/history'
...
async function loadData() {
  ...
  try {
    ...
    const response = await getDeviceRealtimeAlarms(params)

    if (response.success && response.code === 200) {
      alarmList.value = response.result?.data || []
      pagination.value.total = response.result?.total || 0
    } else {
      ElMessage.error(response.message || '加载实时告警失败')
      alarmList.value = []
