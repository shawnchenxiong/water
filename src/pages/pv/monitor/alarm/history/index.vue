      ...
      <div class="history-alarm-content">
        ...
        <div class="filter-bar" :class="{ 'mobile-filter-bar': isMobile }">
          <el-form :inline="!isMobile" :model="queryForm" :label-width="isMobile ? '80px' : 'auto'">
            <div class="filter-grid" :class="{ 'mobile-filter-grid': isMobile }">
              ...
              <!-- 高级筛选项（移动端可折叠） -->
              <el-form-item label="告警消除时间" class="date-range-item">
                <el-date-picker
        ...
        <div class="table-container">
          ...
          >
            ...
            <el-table-column prop="deviceName" label="告警设备" width="130">
              <template #default="{ row }">
                <el-button 
                  link 
                  type="primary" 
            ...
            <el-table-column prop="time" label="告警产生时间" width="160" />
            <el-table-column prop="eliminationTime" label="告警消除时间" width="160" />
...
function checkSelectable(row: OperationLogItem) {
...
async function handleBatchEliminate() {
  // 筛选出可以消除的告警（未消除且没有消除时间）
  const eliminatableRows = selectedRows.value.filter(row => 
    row.isConfirm !== 2 && !row.eliminationTime
  )
  ...
  try {
    ...
    if (eliminatePromises.length === 0) {
      loading.value = false
      ElMessage.warning('所选告警中没有有效的ID')
      return
    }
