...
mxCell.prototype.removeAttribute = function (name) {
    ...
    null != userObject && userObject.nodeType == mxConstants.NODETYPE_ELEMENT && userObject.removeAttribute(name);
};
mxCell.prototype.clone = function () {
    var clone = mxUtils.clone(this, this.mxTransient);
    return clone.setValue(this.cloneValue()), clone;
};

/**
 * 构造一个新的图实例。请注意，构造函数不采用
 * 容器，因为创建 UI 需要图形实例，这
 * 反过来将为图表创建容器。因此，容器是
 * 稍后在 EditorUi 中分配。
 */
...
function Graph(container, model, renderHint, stylesheet, themes, standalone) {
    // 设置页main 默认尺寸
    mxGraph.prototype.pageFormat = new mxRectangle(0, 0, 1920, 1080);
    ...
    // 通过样式添加对 HTML 标签的支持。注：目前仅支持Java
    // 后端支持 HTML 标签，但 CSS 支持仅限于以下内容：
    // http://docs.oracle.com/javase/6/docs/api/index.html?javax/swing/text/html/CSS.html
    // TODO：换行不应影响 isHtmlLabel 输出（应稍后处理）
    ...
    if (this.edgeMode) {
        ...
        // 使用此事件处理 mouseDown 以在更改之前检查选择状态
        this.addListener(
                    ...
                    if (!mxEvent.isAltDown(me.getEvent()) && state != null) {
                        ...
                        if (this.model.isEdge(state.cell)) {
                            ...
                            if (handler == null || handler.getHandleForEvent(me) == null) {
                                ...
                                box.grow(mxEvent.isTouchEvent(me.getEvent()) ? mxShape.prototype.svgStrokeTolerance - 1 : mxShape.prototype.svgStrokeTolerance / 2);

                                if (this.isTableCell(state.cell)) {
                                    const row = this.model.getParent(state.cell);
                                    const table = this.model.getParent(row);

                                    if ((mxUtils.intersects(box, new mxRectangle(state.x, state.y - 1, state.width, 1)) && this.model.getChildAt(table, 0) != row) || mxUtils.intersects(box, new mxRectangle(state.x, state.y + state.height - 1, state.width, 1)) || (mxUtils.intersects(box, new mxRectangle(state.x - 1, state.y, 1, state.height)) && this.model.getChildAt(row, 0) != state.cell) || mxUtils.intersects(box, new mxRectangle(state.x + state.width - 1, state.y, 1, state.height))) {
                                ...
                                while (!me.isConsumed() && current != null && (this.isTableCell(current.cell) || this.isTableRow(current.cell) || this.isTable(current.cell))) {
                                    if (this.isSwimlane(current.cell)) {
                                        ...
                                        if (((offset.x > 0 || offset.width > 0) && mxUtils.intersects(box, new mxRectangle(current.x + offset.x - offset.width - 1 + (offset.x == 0 ? current.width : 0), current.y, 1, current.height))) || ((offset.y > 0 || offset.height > 0) && mxUtils.intersects(box, new mxRectangle(current.x, current.y + offset.y - offset.height - 1 + (offset.y == 0 ? current.height : 0), current.width, 1)))) {
                                            ...
                                            if (pts != null && ((edgeStyle == null && handle == null) || edgeStyle == mxEdgeStyle.OrthConnector)) {
                                                ...
                                                if (handle == null) {
                                                    ...
                                                    if (mxUtils.contains(box, pts[0].x, pts[0].y)) {
                                                        ...
                                                        const nobends = edgeStyle != null && (pts.length == 2 || (pts.length == 3 && ((Math.round(pts[0].x - pts[1].x) == 0 && Math.round(pts[1].x - pts[2].x) == 0) || (Math.round(pts[0].y - pts[1].y) == 0 && Math.round(pts[1].y - pts[2].y) == 0))));
                        ...
                        if (state != null) {
                            ...
                            if (this.model.isEdge(state.cell)) {
                                ...
                                if (this.isTableCell(state.cell)) {
                                    ...
                                    if ((mxUtils.intersects(box, new mxRectangle(state.x - 1, state.y, 1, state.height)) && this.model.getChildAt(row, 0) != state.cell) || mxUtils.intersects(box, new mxRectangle(state.x + state.width - 1, state.y, 1, state.height))) {
                                        cursor = 'col-resize';
                                    } else if ((mxUtils.intersects(box, new mxRectangle(state.x, state.y - 1, state.width, 1)) && this.model.getChildAt(table, 0) != row) || mxUtils.intersects(box, new mxRectangle(state.x, state.y + state.height - 1, state.width, 1))) {
                                        cursor = 'row-resize';
                                    }
                                }
                                ...
                                while (cursor == null && current != null && (this.isTableCell(current.cell) || this.isTableRow(current.cell) || this.isTable(current.cell))) {
                                    if (this.isSwimlane(current.cell)) {
                                        ...
                                        if ((offset.x > 0 || offset.width > 0) && mxUtils.intersects(box, new mxRectangle(current.x + offset.x - offset.width - 1 + (offset.x == 0 ? current.width : 0), current.y, 1, current.height))) {
                                            cursor = 'col-resize';
                                        } else if ((offset.y > 0 || offset.height > 0) && mxUtils.intersects(box, new mxRectangle(current.x, current.y + offset.y - offset.height - 1 + (offset.y == 0 ? current.height : 0), current.width, 1))) {
                                            cursor = 'row-resize';
                                        }
                                    }
    ...
    this.cellRenderer.getLabelValue = function (state) {
        ...
        if (state.view.graph.isHtmlLabel(state.cell)) {
            if (state.style['html'] != 1) {
                ...
                if (result && result.indexOf('<table') == 0 && state.cell && state.cell.getId()) {
                    var div = document.createElement('div');
                    div.innerHTML = result;
                    var node = div.childNodes[0];
                    if (!node.id) {
                        node.setAttribute('id', 'usr_table_' + state.cell.getId());
                        result = div.textContent || div.innerText || div.innerHTML || '';
                    }
                }
    ...
    if (typeof mxVertexHandler !== 'undefined') {
        ...
        this.graphHandler.createPreviewShape = function () {
        ...
        this.graphHandler.getCells = function () {
            ...
            for (let i = 0; i < cells.length; i++) {
                ...
                if (this.graph.isPart(cell)) {
                    const parent = this.graph.getCompositeParent(cell);
                    if (parent != null) {
                        // 仅当 cell 确实是零件时，才重定向到复合父级
                        if (this.graph.isTableRow(cell)) {
                            cell = !this.graph.isCellSelected(cell) ? parent : cell;
                        } else {
                            cell = parent;
                        }
                    }
        ...
        this.graphHandler.start = function (cell) {
            if (this.graph.isPart(cell)) {
                arguments[0] = this.graph.getCompositeParent(cell);
            }
        ...
        this.connectionHandler.createTargetVertex = function (evt, source) {
            ...
            return mxConnectionHandler.prototype.createTargetVertex.call(this, evt, source);
        };
        ...
        this.connectionHandler.isOutlineConnectEvent = function (me) {
            return (this.currentState != null && me.getState() == this.currentState && timeOnTarget > 2000) || ((this.currentState == null || mxUtils.getValue(this.currentState.style, 'outlineConnect', '1') != '0') && connectionHandleIsOutlineConnectEvent.apply(this, arguments));
        };
        ...
        // 重定向选择逻辑：点击零件时自动选择其复合父级（方便整体拖动），
        // 但允许双击零件以进入编辑模式（满足用户编辑变量名或单位的需求）。
        const graphSelectCellForEvent = this.selectCellForEvent;
        this.selectCellForEvent = function (cell, evt) {
            if (cell != null && this.isPart(cell) && mxEvent.getClickCount(evt) < 2) {
                cell = this.getCompositeParent(cell);
            }
            graphSelectCellForEvent.call(this, cell, evt);
        };
        ...
        rubberband.isForceRubberbandEvent = function (me) {
            return (isForceRubberBandEvent.apply(this, arguments) && !mxEvent.isShiftDown(me.getEvent()) && !mxEvent.isControlDown(me.getEvent())) || (mxClient.IS_CHROMEOS && mxEvent.isShiftDown(me.getEvent())) || (mxUtils.hasScrollbars(this.graph.container) && mxClient.IS_FF && mxClient.IS_WIN && me.getState() == null && mxEvent.isTouchEvent(me.getEvent()));
        };
        ...
        this.click = function (me) {
            const locked = me.state == null && me.sourceState != null && this.isCellLocked(me.sourceState.cell);
        ...
        this.graphHandler.shouldRemoveCellsFromParent = function (parent) {
        ...
        this.addListener(
            ...
            mxUtils.bind(this, function (sender, evt) {
                if (!mxEvent.isMultiTouchEvent(evt)) {
                    ...
                    if (cell == null) {
                        const pt = mxUtils.convertPoint(this.container, mxEvent.getClientX(me), mxEvent.getClientY(me));
...
Graph.createSvgImage = function (w, h, data, coordWidth, coordHeight) {
    const tmp = unescape(encodeURIComponent('<!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN" "http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd">' + '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="' + w + 'px" height="' + h + 'px" ' + (coordWidth != null && coordHeight != null ? 'viewBox="0 0 ' + coordWidth + ' ' + coordHeight + '" ' : '') + 'version="1.1">' + data + '</svg>'));
...
Graph.compress = function (data, deflate) {
    ...
    const tmp = deflate ? pako.deflate(encodeURIComponent(data), { to: 'string' }) : pako.deflateRaw(encodeURIComponent(data), { to: 'string' });
    return btoa(tmp);
};
...
(function () {
    ...
    Graph.prototype.getCellAt = function (x, y) {
    ...
    /*Graph.prototype.isCellVisible = function (cell) {
        ...
        if (result && state != null) {
            ...
            let visible = mxUtils.getValue(style, 'visible', null)
            result = mxUtils.isNullOrUndefined(visible) || visible != '0';
        }
        return result;
    };*/
    ...
    Graph.prototype.isRecursiveVertexResize = function (state) {
        return !this.isSwimlane(state.cell) && this.model.getChildCount(state.cell) > 0 && !this.isCellCollapsed(state.cell) && mxUtils.getValue(state.style, 'recursiveResize', '1') == '1' && mxUtils.getValue(state.style, 'childLayout', null) == null;
    };

    const graphIsCellSelected = Graph.prototype.isCellSelected;
    Graph.prototype.isCellSelected = function (cell) {
        if (cell != null && this.isPart(cell)) {
            const parent = this.getCompositeParent(cell);
            if (parent != null && parent !== cell) {
                return graphIsCellSelected.call(this, parent);
            }
        }
        return graphIsCellSelected.call(this, cell);
    };

    const graphIsCellMovable = Graph.prototype.isCellMovable;
    Graph.prototype.isCellMovable = function (cell) {
        if (cell != null && this.isPart(cell)) {
            const parent = this.getCompositeParent(cell);
            if (parent != null && parent !== cell) {
                return this.isCellMovable(parent);
            }
        }
        return graphIsCellMovable.apply(this, arguments);
    };

    const graphIsCellResizable = Graph.prototype.isCellResizable;
    Graph.prototype.isCellResizable = function (cell) {
        if (cell != null && this.isPart(cell)) {
            const parent = this.getCompositeParent(cell);
            if (parent != null && parent !== cell) {
                return this.isCellResizable(parent);
            }
        }
        return graphIsCellResizable.apply(this, arguments);
    };

    const graphIsCellDeletable = Graph.prototype.isCellDeletable;
    Graph.prototype.isCellDeletable = function (cell) {
        if (cell != null && this.isPart(cell)) {
            const parent = this.getCompositeParent(cell);
            if (parent != null && parent !== cell) {
                return this.isCellDeletable(parent);
            }
        }
    ...
    Graph.prototype.isPart = function (cell) {
        return cell != null && (mxUtils.getValue(this.getCellStyle(cell), 'part', '0') == '1' || this.isTableCell(cell) || this.isTableRow(cell));
    };

    Graph.prototype.getCompositeParent = function (cell) {
        if (cell == null) return null;
        let current = cell;
        while (this.isPart(current)) {
            const temp = this.model.getParent(current);
            if (temp == null || !this.model.isVertex(temp)) {
                break;
            }
            current = temp;
        }
    ...
    mxCellHighlight.prototype.getStrokeWidth = function () {
    ...
    const graphIsCellEditable = Graph.prototype.isCellEditable;

    Graph.prototype.isCellEditable = function (cell) {
        let style = this.getCellStyle(cell);
        if (style['readonly'] == '1' || style['erchenchiType'] === 'statusIndicator') {
            return false;
        }
        return graphIsCellEditable.apply(this, arguments);
    };
...
Graph.prototype.customLinkClicked = function () {
    return false;
};
    ...
    this.layoutManager.getLayout = function (cell, eventName) {
        ...
        if (eventName != mxEvent.BEGIN_UPDATE || this.hasLayout(parent, eventName)) {
            ...
            if (style['childLayout'] == 'stackLayout') {
                ...
                const flowLayout = new mxHierarchicalLayout(this.graph, mxUtils.getValue(style, 'flowOrientation', mxConstants.DIRECTION_EAST));
...
Graph.prototype.getPageLayout = function () {
    ...
    if (bounds.width == 0 || bounds.height == 0) {
        ...
        const x0 = Math.floor(Math.ceil(bounds.x / this.view.scale - this.view.translate.x) / size.width);
        const y0 = Math.floor(Math.ceil(bounds.y / this.view.scale - this.view.translate.y) / size.height);
        const w0 = Math.ceil((Math.floor((bounds.x + bounds.width) / this.view.scale) - this.view.translate.x) / size.width) - x0;
        const h0 = Math.ceil((Math.floor((bounds.y + bounds.height) / this.view.scale) - this.view.translate.y) / size.height) - y0;
    ...
    try {
        ...
        for (let i = 0; i < cells.length; i++) {
            if (this.model.isEdge(cells[i])) {
                ...
                if (state != null) {
                    this.flipEdgePoints(cells[i], horizontal, (horizontal ? state.getCenterX() : state.getCenterY()) / this.view.scale - (horizontal ? state.origin.x : state.origin.y) - (horizontal ? this.view.translate.x : this.view.translate.y));
                }
            } else {
...
Graph.prototype.getLabel = function (cell) {
    ...
    // 二沉池: 数据显示组件专用标签渲染逻辑
    if (style['erchenchiType'] === 'dataDisplay') {
        const title = style['dataTitle'] || '参数名称';
        const value = (result != null && result !== '') ? result : '0.00';

        // 获取自定义样式属性
        const titleFontSize = style['dataTitleFontSize'] || '12';
        const titleColor = style['dataTitleFontColor'] || '#ffffff';
        const titleSpacing = style['dataTitleSpacing'] || '0';

        const valueFontSize = style['dataValueFontSize'] || '13';
        const valueColor = style['dataValueFontColor'] || '#00ddff';
        const valueSpacing = style['dataValueSpacing'] || '8';

        // 使用 flex 布局实现左侧名称、右侧数值的效果，并支持独立配置样式
        return `<div style="display:flex;justify-content:space-between;align-items:center;width:100%;height:100%;padding:0 5px;box-sizing:border-box;white-space:nowrap;overflow:hidden;">` +
            `<span style="flex:1;text-align:left;overflow:hidden;text-overflow:ellipsis;color:${titleColor};font-size:${titleFontSize}px;margin-left:${titleSpacing}px;margin-right:${titleSpacing}px;">${title}</span>` +
            `<span style="font-weight:bold;color:${valueColor};font-size:${valueFontSize}px;margin-left:${valueSpacing}px;margin-right:${valueSpacing}px;">${value}</span>` +
            `</div>`;
    }

    // 禁止 HTML Input 形状渲染默认 Label（避免重叠）
    if (style['shape'] === 'mxgraph.rc.mxRc_htmlInput') {
        return null;
    }
...
Graph.prototype.isLabelMovable = function (cell) {
    ...
    return !this.isCellLocked(cell) && ((this.model.isEdge(cell) && this.edgeLabelsMovable) || (this.model.isVertex(cell) && (this.vertexLabelsMovable || mxUtils.getValue(style, 'labelMovable', '0') == '1')));
};
...
Graph.prototype.createLayersDialog = function () {
    ...
    for (let i = 0; i < childCount; i++) {
        mxUtils.bind(this, function (layer) {
            ...
            const title = this.convertValueToString(layer) || mxResources.get('background') || 'Background';
...
Graph.prototype.connectVertex = function (source, direction, length, evt, forceClone, ignoreCellAt) {
    ...
    const pt = source.geometry.relative && source.parent.geometry != null ? new mxPoint(source.parent.geometry.width * source.geometry.x, source.parent.geometry.height * source.geometry.y) : new mxPoint(composite.geometry.x, composite.geometry.y);
    ...
    let target = ignoreCellAt || (mxEvent.isControlDown(evt) && !forceClone) ? null : this.getCellAt(dx + pt.x * s, dy + pt.y * s);
    ...
    try {
        ...
        const edge = (mxEvent.isControlDown(evt) && duplicate) || (target == null && layout != null && layout.constructor == mxStackLayout) ? null : this.insertEdge(this.model.getParent(source), null, '', source, realTarget, this.createCurrentEdgeStyle());
...
Graph.prototype.updateAlternateBounds = function (cell, geo) {
...
Graph.prototype.getSwimlaneAt = function () {
...
Graph.prototype.isCellFoldable = function (cell) {
    ...
    return this.foldingEnabled && (style['treeFolding'] == '1' || (!this.isCellLocked(cell) && ((this.isContainer(cell) && style['collapsible'] != '0') || (!this.isContainer(cell) && style['collapsible'] == '1'))));
};
...
Graph.prototype.compressNode = function (node, checked) {
    return Graph.compressNode(node, checked);
};
...
Graph.prototype.decompress = function (data) {
    return Graph.decompress(data);
};

Graph.decompress = function (data) {
    if (null == data || 0 == data.length || 'undefined' == typeof pako) return data;
    var tmp = window.atob ? atob(data) : Base64.decode(data, !0);
    return Graph.zapGremlins(decodeURIComponent(Graph.bytesToString(pako.inflateRaw(tmp))));
};
...
Graph.prototype.getDefaultColor = function (style, key, defaultValue, inverseDefaultValue) {
    let temp = 'default' + key.charAt(0).toUpperCase() + key.substring(1);
...
Graph.prototype.defaultDirections = [{
    key: mxConstants.DIRECTION_NORTH,
    title: '向上'
}, { key: mxConstants.DIRECTION_SOUTH, title: '向下' }, {
    key: mxConstants.DIRECTION_EAST,
    title: '向右'
}, { key: mxConstants.DIRECTION_WEST, title: '向左' }];
 ...
 * Up arrow.
 */
HoverIcons.prototype.triangleUp = Graph.createSvgImage(18, 28, '<path d="m 6 26 L 12 26 L 12 12 L 18 12 L 9 1 L 1 12 L 6 12 z" ' + 'stroke="#fff" fill="' + HoverIcons.prototype.arrowFill + '"/>');

/**
 * Right arrow.
 */
HoverIcons.prototype.triangleRight = Graph.createSvgImage(26, 18, '<path d="m 1 6 L 14 6 L 14 1 L 26 9 L 14 18 L 14 12 L 1 12 z" ' + 'stroke="#fff" fill="' + HoverIcons.prototype.arrowFill + '"/>');

/**
 * Down arrow.
 */
HoverIcons.prototype.triangleDown = Graph.createSvgImage(18, 26, '<path d="m 6 1 L 6 14 L 1 14 L 9 26 L 18 14 L 12 14 L 12 1 z" ' + 'stroke="#fff" fill="' + HoverIcons.prototype.arrowFill + '"/>');

/**
 * Left arrow.
 */
HoverIcons.prototype.triangleLeft = Graph.createSvgImage(28, 18, '<path d="m 1 9 L 12 1 L 12 6 L 26 6 L 26 12 L 12 12 L 12 18 z" ' + 'stroke="#fff" fill="' + HoverIcons.prototype.arrowFill + '"/>');
 ...
 *
 */
HoverIcons.prototype.isResetEvent = function (evt) {
...
HoverIcons.prototype.click = function (state, dir, me) {
    ...
    if (tmp != null && this.graph.model.isEdge(tmp.cell) && !mxEvent.isControlDown(evt) && (tmp.getVisibleTerminalState(true) == state || tmp.getVisibleTerminalState(false) == state)) {
        ...
        this.graph.selectCellsForConnectVertex(this.graph.connectVertex(state.cell, dir, this.graph.defaultEdgeLength, evt), evt, this);
    }
...
HoverIcons.prototype.repaint = function () {
    ...
    if (this.currentState != null) {
        ...
        if (this.currentState != null) {
            if (mxUtils.getValue(this.currentState.style, 'ctrlPanel', '0') == '1' || mxUtils.getValue(this.currentState.style, 'divider', '0') == '1') {
                this.setDisplay('none');
                return;
            }
        }
        ...
        if (this.currentState != null && this.graph.model.isVertex(this.currentState.cell) && this.graph.isCellConnectable(this.currentState.cell)) {
            ...
            if (this.checkCollisions) {
                ...
                const checkCollision = mxUtils.bind(this, function (cell, arrow) {
                    ...
                    if (cell != null && !this.graph.model.isAncestor(cell, this.currentState.cell) && !this.graph.isSwimlane(cell) && (geo == null || currentGeo == null || (geo.height < 3 * currentGeo.height && geo.width < 3 * currentGeo.width))) {
...
Graph.prototype.createTable = function (rowCount, colCount, w, h, title, startSize, tableStyle, rowStyle, cellStyle) {
    ...
    return this.createParent(this.createVertex(null, null, title != null ? title : '', 0, 0, colCount * w, rowCount * h + (title != null ? startSize : 0), tableStyle), this.createParent(this.createVertex(null, null, '', 0, 0, colCount * w, h, rowStyle), this.createVertex(null, null, '', 0, 0, w, h, cellStyle), colCount, w, 0), rowCount, 0, h);
};
...
Graph.prototype.createCrossFunctionalSwimlane = function (rowCount, colCount, w, h, title, tableStyle, rowStyle, firstCellStyle, cellStyle) {
    ...
    rowStyle = rowStyle != null ? rowStyle : 'shape=tableRow;horizontal=0;swimlaneHead=0;swimlaneBody=0;top=0;left=0;strokeColor=inherit;' + 'bottom=0;right=0;dropTarget=0;fontStyle=0;fillColor=none;points=[[0,0.5],[1,0.5]];portConstraint=eastwest;startSize=40;' + s;
    firstCellStyle = firstCellStyle != null ? firstCellStyle : 'swimlane;swimlaneHead=0;swimlaneBody=0;fontStyle=0;strokeColor=inherit;' + 'connectable=0;fillColor=none;startSize=40;' + s;
...
Graph.prototype.visitTableCells = function (cell, visitor) {
    ...
    for (var i = 0; i < rows.length; i++) {
        ...
        for (var j = 0; j < cols.length; j++) {
            ...
            col.point = new mxPoint(geo.width + (lastCol != null ? lastCol.point.x : start.x + rowStart.x), geo.height + (lastRow != null && lastRow[0] != null ? lastRow[0].point.y : start.y + rowStart.y));
            col.actual = col;
            ...
            var head = mxUtils.getValue(rowStyle, mxConstants.STYLE_SWIMLANE_HEAD, 1) == 1 && mxUtils.getValue(rowStyle, mxConstants.STYLE_STROKECOLOR, mxConstants.NONE) != mxConstants.NONE;
...
Graph.prototype.createViewState = function (node) {
    ...
    return {
        ...
        pageFormat: !isNaN(pw) && !isNaN(ph) ? new mxRectangle(0, 0, pw, ph) : typeof mxSettings === 'undefined' || this.defaultPageFormat != null ? mxGraph.prototype.pageFormat : mxSettings.getPageFormat(),
...
(function () {
    ...
    mxGraphView.prototype.validateCellState = function (cell, recurse) {
        ...
        if (state != null && recurse && this.graph.model.isEdge(state.cell) && state.style != null && state.style[mxConstants.STYLE_CURVED] != 1 && !state.invalid && this.updateLineJumps(state)) {
    ...
    mxCellRenderer.prototype.isShapeInvalid = function (state, shape) {
        return mxCellRendererIsShapeInvalid.apply(this, arguments) || (state.routedPoints != null && shape.routedPoints != null && !mxUtils.equalPoints(shape.routedPoints, state.routedPoints));
    };
    ...
    mxConnector.prototype.paintLine = function (c, absPts, rounded) {
        ...
        if (this.outline || this.state == null || this.style == null || this.state.routedPoints == null || this.state.routedPoints.length == 0) {
            ...
            const size = (parseInt(mxUtils.getValue(this.style, 'jumpSize', Graph.defaultJumpSize)) - 2) / 2 + this.strokewidth;
...
mxStencilRegistry.getStencil = function (name) {
    ...
    if (result == null && mxCellRenderer.defaultShapes[name] == null && mxStencilRegistry.dynamicLoading) {
        ...
        if (basename != null) {
            ...
            if (libs != null) {
                let pn = mxStencilRegistry.packages[basename];
                if (pn == null) {
...
if (typeof mxVertexHandler != 'undefined') {
    (function () {
        ...
        mxConnectionHandler.prototype.createMarker = function () {
            ...
            marker.getCell = mxUtils.bind(this, function () {
        ...
        Graph.prototype.importGraphModel = function (node, dx, dy, crop) {
            ...
            let layers = tempModel.getChildren(this.cloneCell(tempModel.root, this.isCloneInvalidEdges(), cloneMap));
                ...
                try {
                    ...
                    if (layers.length == 1 && !this.isCellLocked(this.getDefaultParent())) {
                        ...
                        for (let i = 0; i < layers.length; i++) {
                            const children = this.model.getChildren(this.moveCells([layers[i]], dx, dy, false, this.model.getRoot())[0]);
         ...
         */
        Graph.prototype.updateCustomLinksForCell = function () {
            // Hook for subclassers
        };
        ...
        Graph.prototype.flipEdge = function (edge) {
            if (edge != null) {
                ...
                const elbow = mxUtils.getValue(style, mxConstants.STYLE_ELBOW, mxConstants.ELBOW_HORIZONTAL);
                const value = elbow == mxConstants.ELBOW_HORIZONTAL ? mxConstants.ELBOW_VERTICAL : mxConstants.ELBOW_HORIZONTAL;
        ...
        Graph.prototype.isValidDropTarget = function (cell, cells) {
            ...
            return (mxUtils.getValue(style, 'part', '0') != '1' || this.isContainer(cell)) && mxUtils.getValue(style, 'dropTarget', '1') != '0' && (mxGraph.prototype.isValidDropTarget.apply(this, arguments) || this.isContainer(cell)) && !this.isTableRow(cell) && (!this.isTable(cell) || rows);
        };
         ...
         */
        Graph.prototype.getPreferredSizeForCell = function () {
                ...
                for (let i = 0; i < cells.length; i++) {
                    ...
                    if (model.isEdge(cell)) {
                        ...
                        if (geo != null) {
                            ...
                            if (state != null) {
                                const dirs = [mxConstants.DIRECTION_EAST, mxConstants.DIRECTION_SOUTH, mxConstants.DIRECTION_WEST, mxConstants.DIRECTION_NORTH];
        ...
        Graph.prototype.processChange = function (change) {
            if (change.constructor.name === 'mxGeometryChange' && (this.isTableCell(change.cell) || this.isTableRow(change.cell)) && ((change.previous == null && change.geometry != null) || (change.previous != null && !change.previous.equals(change.geometry)))) {
            ...
            if (change.constructor.name === 'mxValueChange' && change.cell != null && change.cell.value != null && typeof change.cell.value == 'object') {
        ...
        Graph.prototype.getAttributeForCell = function (cell, attributeName, defaultValue) {
            const value = cell.value != null && typeof cell.value === 'object' ? cell.value.getAttribute(attributeName) : null;
        ...
        Graph.prototype.getDropTarget = function (cells, evt) {
        ...
        Graph.prototype.dblClick = function (evt, cell) {
            if ((!cell || ('1' != cell.getAttribute('isUsrData') && '1' != cell.getAttribute('isUsrPanel') && '1' != cell.getAttribute('isFunc')) || '0' != cell.getAttribute('isEnterText')) && this.isEnabled()) {
                var pt = mxUtils.convertPoint(this.container, mxEvent.getClientX(evt), mxEvent.getClientY(evt));
                if (null != evt && !this.model.isVertex(cell)) {
                    var state = this.model.isEdge(cell) ? this.view.getState(cell) : null,
                        src = mxEvent.getSource(evt);
                    if (this.firstClickState == state && this.firstClickSource == src && (null == state || null == state.text || null == state.text.node || (!mxUtils.contains(state.text.boundingBox, pt.x, pt.y) && !mxUtils.isAncestorNode(state.text.node, mxEvent.getSource(evt)))) && ((null == state && !this.isCellLocked(this.getDefaultParent())) || (null != state && !this.isCellLocked(state.cell))) && (null != state || (mxClient.IS_VML && src == this.view.getCanvas()) || (mxClient.IS_SVG && src == this.view.getCanvas().ownerSVGElement))) return;
                }
        ...
        Graph.prototype.getFreeInsertPoint = function () {
            ...
            const x = this.snap(Math.round(Math.max(pt.x, bds.x / view.scale - view.translate.x + (bds.width == 0 ? 2 * this.gridSize : 0))));
            const y = this.snap(Math.round(Math.max(pt.y, (bds.y + bds.height) / view.scale - view.translate.y + 2 * this.gridSize)));
        ...
        Graph.prototype.getCenterInsertPoint = function (bbox) {
            ...
            if (mxUtils.hasScrollbars(this.container)) {
                return new mxPoint(this.snap(Math.round((this.container.scrollLeft + this.container.clientWidth / 2) / this.view.scale - this.view.translate.x - bbox.width / 2)), this.snap(Math.round((this.container.scrollTop + this.container.clientHeight / 2) / this.view.scale - this.view.translate.y - bbox.height / 2)));
            } else {
                return new mxPoint(this.snap(Math.round(this.container.clientWidth / 2 / this.view.scale - this.view.translate.x - bbox.width / 2)), this.snap(Math.round(this.container.clientHeight / 2 / this.view.scale - this.view.translate.y - bbox.height / 2)));
            }
        };
        ...
        Graph.prototype.addClickHandler = function (highlight, beforeClick, onClick) {
            ...
            const mouseListener = {
                ...
                updateCurrentState: function (me) {
                    ...
                    if (tmp == null || graph.getLinkForCell(tmp.cell) == null) {
                        const cell = graph.getCellAt(me.getGraphX(), me.getGraphY(), null, null, null, function (state) {
                ...
                mouseUp: function (sender, me) {
                    ...
                    if (linkNode == null && Math.abs(this.scrollLeft - graph.container.scrollLeft) < tol && Math.abs(this.scrollTop - graph.container.scrollTop) < tol && (me.sourceState == null || !me.isSource(me.sourceState.control)) && (((mxEvent.isLeftMouseButton(evt) || mxEvent.isMiddleMouseButton(evt)) && !mxEvent.isPopupTrigger(evt)) || mxEvent.isTouchEvent(evt))) {
                        if (this.currentLink != null) {
                            ...
                            if (!mxEvent.isConsumed(evt)) {
                                const target = mxEvent.isMiddleMouseButton(evt) ? '_blank' : blank ? graph.linkTarget : '_top';
            ...
            mxEvent.addListener(document, 'mouseleave', function () {
                mouseListener.clear();
            });
        };
        ...
        Graph.prototype.isCellResizable = function (cell) {
            ...
            return !this.isTableCell(cell) && !this.isTableRow(cell) && (result || (mxUtils.getValue(style, mxConstants.STYLE_RESIZABLE, '1') != '0' && style[mxConstants.STYLE_WHITE_SPACE] == 'wrap'));
        };
        ...
        Graph.prototype.getSvg = function (background, scale, border, nocrop, crisp, ignoreSelection, showText, imgExport, linkTarget, hasShadow) {
            ...
            try {
                ...
                const bounds = ignoreSelection || nocrop ? this.getGraphBounds() : this.getBoundingBox(this.getSelectionCells());
                ...
                imgExport.getLinkForCellState = function (state) {
                ...
                imgExport.drawCellState = function (state) {
                    const graph = state.view.graph;
        ...
        Graph.prototype.insertRow = function (table, index) {
            for (var bd = table.tBodies[0], cells = bd.rows[0].cells, cols = 0, i = 0; i < cells.length; i++) {
                var colspan = cells[i].getAttribute('colspan');
                cols += null != colspan ? parseInt(colspan) : 1;
            }
            var row = bd.insertRow(index);
            for (i = 0; i < cols; i++) mxUtils.br(row.insertCell(-1));
            return row.cells[0];
        };
        Graph.prototype.deleteRow = function (table, index) {
            table && table.tBodies[0] && table.tBodies[0].rows.length > 1 && table.tBodies[0].deleteRow(index);
        };
        Graph.prototype.insertColumn = function (table, index) {
            ...
            if (null != hd)
                for (var h = 0; h < hd.rows.length; h++) {
                    ...
                    hd.rows[h].appendChild(th), mxUtils.br(th);
                }
            for (var bd = table.tBodies[0], i = 0; i < bd.rows.length; i++) {
        ...
        Graph.prototype.deleteColumn = function (table, index) {
            if (index >= 0) for (var rows = table.tBodies[0].rows, i = 0; i < rows.length; i++) rows[i].cells.length > index && rows[i].deleteCell(index);
        };
        ...
        Graph.prototype.initTouch = function () {
            ...
            this.popupMenuHandler.mouseUp = mxUtils.bind(this, function (sender, me) {
                this.popupMenuHandler.popupTrigger = !this.isEditing() && this.isEnabled() && (me.getState() == null || !me.isSource(me.getState().control)) && (this.popupMenuHandler.popupTrigger || (!menuShowing && !mxEvent.isMouseEvent(me.getEvent()) && ((selectionEmpty && me.getCell() == null && this.isSelectionEmpty()) || (cellSelected && this.isCellSelected(me.getCell())))));
        ...
        Graph.prototype.handleCustomLink = function (href) {
            if ('data:action/json,' == href.substring(0, 17)) {
                ...
                if (null != action.actions) {
                    for (var i = 0; i < action.actions.length; i++)
                        if (null != action.actions[i].open)
                            if (this.isCustomLink(action.actions[i].open)) {
                                if (!this.customLinkClicked(action.actions[i].open)) return;
                            } else this.openLink(action.actions[i].open);
                    this.model.beginUpdate();
                    try {
                        for (i = 0; i < action.actions.length; i++) this.handleLinkAction(action.actions[i]);
                    } finally {
                        this.model.endUpdate();
                    }
                }
        ...
        Graph.prototype.getCellsForTags = function (tagList, cells, propertyName) {
            var result = [];
            if (null != tagList) {
                (cells = null != cells ? cells : this.model.getDescendants(this.model.getRoot())), (propertyName = null != propertyName ? propertyName : 'tags');
                for (var i = 0; i < cells.length; i++)
                    if (this.model.isVertex(cells[i]) || this.model.isEdge(cells[i])) {
                        var tags = null != cells[i].value && 'object' == _typeof(cells[i].value) ? mxUtils.trim(cells[i].value.getAttribute(propertyName) || '') : '',
                            match = !0;
                        if (tags.length > 0)
                            for (var tmp = tags.toLowerCase().split(' '), j = 0; j < tagList.length && match; j++) {
                                var tag = mxUtils.trim(tagList[j]).toLowerCase();
                                match = match && (0 == tag.length || mxUtils.indexOf(tmp, tag) >= 0);
                            }
                        else match = 0 == tagList.length;
                        match && result.push(cells[i]);
                    }
            }
            return result;
        };
        Graph.prototype.toggleCells = function (cells) {
            this.model.beginUpdate();
            try {
                for (var i = 0; i < cells.length; i++) this.model.setVisible(cells[i], !this.model.isVisible(cells[i]));
            } finally {
                this.model.endUpdate();
            }
        };
        Graph.prototype.setCellsVisible = function (cells, visible) {
            this.model.beginUpdate();
            try {
                for (var i = 0; i < cells.length; i++) this.model.setVisible(cells[i], visible);
            } finally {
                this.model.endUpdate();
            }
        };
        Graph.prototype.highlightCells = function (cells, color, duration, opacity) {
            for (var i = 0; i < cells.length; i++) this.highlightCell(cells[i], color, duration, opacity);
        };
        Graph.prototype.highlightCell = function (cell, color, duration, opacity) {
            (color = null != color ? color : mxConstants.DEFAULT_VALID_COLOR), (duration = null != duration ? duration : 1e3);
            var state = this.view.getState(cell);
            if (null != state) {
                var sw = Math.max(5, mxUtils.getValue(state.style, mxConstants.STYLE_STROKEWIDTH, 1) + 4),
                    hl = new mxCellHighlight(this, color, sw, !1);
                null != opacity && (hl.opacity = opacity),
                    hl.highlight(state),
                    window.setTimeout(function () {
                        null != hl.shape && (mxUtils.setPrefixedStyle(hl.shape.node.style, 'transition', 'all 1200ms ease-in-out'), (hl.shape.node.style.opacity = 0)),
                            window.setTimeout(function () {
                                hl.destroy();
                            }, 1200);
                    }, duration);
        ...
        Graph.prototype.addSvgShadow = function (svgRoot, group, createOnly) {
            createOnly = null != createOnly && createOnly;
            var svgDoc = svgRoot.ownerDocument,
                filter = null != svgDoc.createElementNS ? svgDoc.createElementNS(mxConstants.NS_SVG, 'filter') : svgDoc.createElement('filter');
            filter.setAttribute('id', this.shadowId);
            var blur = null != svgDoc.createElementNS ? svgDoc.createElementNS(mxConstants.NS_SVG, 'feGaussianBlur') : svgDoc.createElement('feGaussianBlur');
            blur.setAttribute('in', 'SourceAlpha'), blur.setAttribute('stdDeviation', this.svgShadowBlur), blur.setAttribute('result', 'blur'), filter.appendChild(blur);
            var offset = null != svgDoc.createElementNS ? svgDoc.createElementNS(mxConstants.NS_SVG, 'feOffset') : svgDoc.createElement('feOffset');
            offset.setAttribute('in', 'blur'), offset.setAttribute('dx', this.svgShadowSize), offset.setAttribute('dy', this.svgShadowSize), offset.setAttribute('result', 'offsetBlur'), filter.appendChild(offset);
            var flood = null != svgDoc.createElementNS ? svgDoc.createElementNS(mxConstants.NS_SVG, 'feFlood') : svgDoc.createElement('feFlood');
            flood.setAttribute('flood-color', this.svgShadowColor), flood.setAttribute('flood-opacity', this.svgShadowOpacity), flood.setAttribute('result', 'offsetColor'), filter.appendChild(flood);
            var composite = null != svgDoc.createElementNS ? svgDoc.createElementNS(mxConstants.NS_SVG, 'feComposite') : svgDoc.createElement('feComposite');
            composite.setAttribute('in', 'offsetColor'), composite.setAttribute('in2', 'offsetBlur'), composite.setAttribute('operator', 'in'), composite.setAttribute('result', 'offsetBlur'), filter.appendChild(composite);
            var feBlend = null != svgDoc.createElementNS ? svgDoc.createElementNS(mxConstants.NS_SVG, 'feBlend') : svgDoc.createElement('feBlend');
            feBlend.setAttribute('in', 'SourceGraphic'), feBlend.setAttribute('in2', 'offsetBlur'), filter.appendChild(feBlend);
            var defs = svgRoot.getElementsByTagName('defs'),
                defsElt = null;
            return 0 == defs.length ? ((defsElt = null != svgDoc.createElementNS ? svgDoc.createElementNS(mxConstants.NS_SVG, 'defs') : svgDoc.createElement('defs')), null != svgRoot.firstChild ? svgRoot.insertBefore(defsElt, svgRoot.firstChild) : svgRoot.appendChild(defsElt)) : (defsElt = defs[0]), defsElt.appendChild(filter), createOnly || ((group || svgRoot.getElementsByTagName('g')[0]).setAttribute('filter', 'url(#' + this.shadowId + ')'), isNaN(parseInt(svgRoot.getAttribute('width'))) || (svgRoot.setAttribute('width', parseInt(svgRoot.getAttribute('width')) + 6), svgRoot.setAttribute('height', parseInt(svgRoot.getAttribute('height')) + 6))), filter;
        };
        ...
        mxCellEditor.prototype.installListeners = function () {
        ...
        mxCellEditor.prototype.resize = function (state) {
            if (this.textarea != null) {
        ...
        mxCellEditor.prototype.getBackgroundColor = function (state) {
            ...
            if ((color == null || color == mxConstants.NONE) && state.cell.geometry != null && state.cell.geometry.width > 0 && (mxUtils.getValue(state.style, mxConstants.STYLE_ROTATION, 0) != 0 || mxUtils.getValue(state.style, mxConstants.STYLE_HORIZONTAL, 1) == 0)) {
         ...
         */
        const vertexHandlerCreateParentHighlightShape = mxVertexHandler.prototype.createParentHighlightShape;
        mxVertexHandler.prototype.createParentHighlightShape = function () {
        ...
        mxEdgeHandler.prototype.createParentHighlightShape = function () {
        ...
        mxVertexHandler.prototype.isCenteredEvent = function (state, me) {
            return (!(!this.graph.isSwimlane(state.cell) && this.graph.model.getChildCount(state.cell) > 0 && !this.graph.isCellCollapsed(state.cell) && mxUtils.getValue(state.style, 'recursiveResize', '1') == '1' && mxUtils.getValue(state.style, 'childLayout', null) == null) && mxEvent.isControlDown(me.getEvent())) || mxEvent.isMetaDown(me.getEvent());
        };
         ...
         * Hides rotation handle for table cells and rows.
         */
        const vertexHandlerIsParentHighlightVisible = mxVertexHandler.prototype.isParentHighlightVisible;
         ...
         */
        const vertexHandlerGetSelectionBorderBounds = mxVertexHandler.prototype.getSelectionBorderBounds;
        ...
        if (Graph.touchStyle) {
            ...
            mxPanningHandler.prototype.isPanningTrigger = function (me) {
                ...
                return (me.getState() == null && !mxEvent.isMouseEvent(evt)) || (mxEvent.isPopupTrigger(evt) && (me.getState() == null || mxEvent.isControlDown(evt) || mxEvent.isShiftDown(evt)));
            };
            ...
            mxPanningHandler.prototype.isPanningTrigger = function (me) {
                ...
                return (mxEvent.isLeftMouseButton(evt) && ((this.useLeftButtonForPanning && me.getState() == null) || (mxEvent.isControlDown(evt) && !mxEvent.isShiftDown(evt)))) || (this.usePopupTrigger && mxEvent.isPopupTrigger(evt));
            };
        }
        ...
        mxRubberband.prototype.isSpaceEvent = function (me) {
            return this.graph.isEnabled() && !this.graph.isCellLocked(this.graph.getDefaultParent()) && mxEvent.isControlDown(me.getEvent()) && mxEvent.isShiftDown(me.getEvent());
        };
        ...
        mxEdgeHandler.prototype.isOutlineConnectEvent = function (me) {
            return (this.currentTerminalState != null && me.getState() == this.currentTerminalState && timeOnTarget > 2000) || ((this.currentTerminalState == null || mxUtils.getValue(this.currentTerminalState.style, 'outlineConnect', '1') != '0') && mxEdgeHandlerIsOutlineConnectEvent.apply(this, arguments));
        };
        ...
        mxEdgeHandler.prototype.createHandleShape = function (index) {
            ...
            const c = index != null && (index == 0 || index >= this.state.absolutePoints.length - 1 || (this.constructor == mxElbowEdgeHandler && index == 2)) ? this.graph.getConnectionConstraint(this.state, terminalState, source) : null;
            const pt = c != null ? this.graph.getConnectionPoint(this.state.getVisibleTerminalState(source), c) : null;
            const img = pt != null ? this.fixedHandleImage : c != null && terminalState != null ? this.terminalHandleImage : this.handleImage;
        ...
        mxVertexHandler.prototype.init = function () {
            ...
            // Draws corner rectangles for single selected table cells and rows
                ...
                for (let i = 0; i < 4; i++) {
                    const shape = new mxRectangleShape(new mxRectangle(0, 0, 6, 6), '#ffffff', mxConstants.HANDLE_STROKECOLOR);
        ...
        mxVertexHandler.prototype.redrawHandles = function () {
            ...
            // Shows rotation handle only if one vertex is selected
            if (this.rotationShape != null && this.rotationShape.node != null) {
                this.rotationShape.node.style.display = this.moveHandles == null && this.graph.getSelectionCount() == 1 && (this.index == null || this.index == mxEvent.ROTATION_HANDLE) ? '' : 'none';
            }
            ...
            if (this.state != null && this.linkHint != null) {
                ...
                const rs = bb != null ? mxUtils.getBoundingBox(this.state, this.state.style[mxConstants.STYLE_ROTATION] || '0') : this.state;
