/* eslint-disable */
import { mxConstants, mxEvent, mxUtils, mxPopupMenu, mxPoint, mxGuide } from '../core/mxgraph';

import { Editor } from './Editor';

export { mxRuler, mxDualRuler };
...
function mxRuler(editorUi, unit, isVertical, isSecondery) {
    ...
    var drawRuler = function () {
        ...
        var rStart = hasPageView
            ? RULER_THICKNESS + (isVertical ? bgPages.y - graph.container.scrollTop : bgPages.x - graph.container.scrollLeft)
        ...
        var rEnd = hasPageView
            ? Math.min(rStart + (isVertical ? bgPages.height : bgPages.width), isVertical ? canvas.height : canvas.width)
            : isVertical
            ? canvas.height
            : canvas.width;
        ...
        for (var i = hasPageView ? rStart : rStart % (step * scale); i <= rEnd; i += step * scale) {
            ...
            if (i < RULER_THICKNESS || current == lastTick) {
                //Prevent wasting time in drawing non-visible/duplicate lines
                continue;
            }
...
function mxDualRuler(editorUi, unit) {
    ...
    var installMenu = mxUtils.bind(this, function (node) {
        ...
        mxEvent.addGestureListeners(
            ...
            null,
            mxUtils.bind(this, function (evt) {
                if (editorUi.editor.graph.isEnabled() && !editorUi.editor.graph.isMouseDown && (mxEvent.isTouchEvent(evt) || mxEvent.isPopupTrigger(evt))) {
