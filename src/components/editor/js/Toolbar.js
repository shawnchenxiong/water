 ...
 * 转化为vue
 */
import { mxConstants, mxResources, mxEvent, mxUtils, mxClient, mxPopupMenu } from '../core/mxgraph';
...
Toolbar.prototype.dropdownImageHtml = '<img border="0" style="position:absolute;right:4px;top:' + 8 + 'px;" src="' + Toolbar.prototype.dropdownImage + '" valign="middle"/>';
...
Toolbar.prototype.init = function () {
	this.addItem('geSprite-copy', 'copy', null, false, true);
	this.addItem('geSprite-paste', 'paste', null, false, true);
	this.addItem('geSprite-cut', 'cut', null, false, true);
	this.addItem('geSprite-undo', 'undo', null, false, true);
	this.addItem('geSprite-redo', 'redo', null, false, true);
	this.addSeparator();
	this.addItem('geSprite-leftAlign', 'leftAlign', null, false, true, 'leftAlignTip');
	this.addItem('geSprite-hCenterAlign', 'hCenterAlign', null, false, true, 'hCenterAlignTip');
	this.addItem('geSprite-rightAlign', 'rightAlign', null, false, true, 'rightAlignTip');
	this.addItem('geSprite-topAlign', 'topAlign', null, false, true, 'topAlignTip');
	this.addItem('geSprite-vCenterAlign', 'vCenterAlign', null, false, true, 'vCenterAlignTip');
	this.addItem('geSprite-bottomAlign', 'bottomAlign', null, false, true, 'bottomAlignTip');
	this.addSeparator();
	this.addItem('geSprite-toFront', 'toFront', null, false, true, 'toFrontTip');
	this.addItem('geSprite-toBack', 'toBack', null, false, true, 'toBackTip');
	this.addSeparator();
	const zoomMenu = this.addMenu('', mxResources.get('zoom') + ' (Alt+Mousewheel)', true, 'viewZoom', null, true);
	zoomMenu.showDisabled = true;
	zoomMenu.style.cssText = 'white-space: nowrap; position: relative; overflow: hidden; width: 50px;';
	this.updateZoom = mxUtils.bind(this, function () {
		zoomMenu.innerHTML = `<div class="zoom-num">${Math.round(this.editorUi.editor.graph.view.scale * 100)}%</div><div class="zoom-label">${mxResources.get('zoom')}</div>${this.dropdownImageHtml}`;
		if (EditorUi.compactUi) {
			zoomMenu.getElementsByTagName('img')[0].style.right = '1px';
			zoomMenu.getElementsByTagName('img')[0].style.top = '10px';
		}
	});
	...
	this.addSeparator();
	this.addItem('geSprite-image', 'image', null, false, true);
	this.addSeparator();
	this.addItem('geSprite-group', 'group', null, false, true);
	this.addItem('geSprite-ungroup', 'ungroup', null, false, true);
	this.addItem('geSprite-lock', 'lock', null, false, true);
	this.addItem('geSprite-unlock', 'unlock', null, false, true);

	var toolbarRight = document.createElement('div');
	toolbarRight.className = 'toolbar-right';
	this.container.appendChild(toolbarRight);
	this.addItem('geSprite-point-set', 'pointSet', toolbarRight, true, true);
	this.addItem('geSprite-save', 'saveContent', toolbarRight, false, true);
	if (this.editorUi.GLOBAL_CONFIG.deviceId) {
		this.addItem('geSprite-preview', 'preview', toolbarRight, false, true);
	}
	...
	if (sw >= 700) {
		const formatMenu = this.addMenu('', mxResources.get('view') + ' (' + mxResources.get('panTooltip') + ')', true, 'viewPanels', null, true);
		this.addDropDownArrow(formatMenu, 'geSprite-formatpanel', 38, 50, -4, -3, 36, -8);
		this.addSeparator();
	}

	const viewMenu = this.addMenu('', mxResources.get('zoom') + ' (Alt+Mousewheel)', true, 'viewZoom', null, true);
	...
	this.updateZoom = mxUtils.bind(this, function () {
		viewMenu.innerHTML = `<div class="zoom-num">${Math.round(this.editorUi.editor.graph.view.scale * 100)}%</div><div class="zoom-label">${mxResources.get('zoom')}</div>${this.dropdownImageHtml}`;
			...
			if (row == null) {
				...
				elt = menu.addItem(
					...
					mxUtils.bind(this, function () {
						...
						const color = table.style.borderColor.replace(/\brgb\s*\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)/g, function ($0, $1, $2, $3) {
				...
				elt = menu.addItem(
					...
					mxUtils.bind(this, function () {
						// Converts rgb(r,g,b) values
						const color = table.style.backgroundColor.replace(/\brgb\s*\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)/g, function ($0, $1, $2, $3) {
							return '#' + ('0' + Number($1).toString(16)).substr(-2) + ('0' + Number($2).toString(16)).substr(-2) + ('0' + Number($3).toString(16)).substr(-2);
						});
						this.editorUi.pickColor(color, function (newColor) {
							if (newColor == null || newColor == mxConstants.NONE) {
 ...
 * Adds given action item
 */
Toolbar.prototype.addItem = function (sprite, key, c, ignoreDisabled, insertLabel, outtooltip) {
	const action = this.editorUi.actions.get(key);
	let elt = null;

	if (action != null) {
		let tooltip = outtooltip ? mxResources.get(outtooltip) : action.label;

		if (!outtooltip && action.shortcut != null) {
			tooltip += ' (' + action.shortcut + ')';
		}

		elt = this.addButton(sprite, tooltip, action.funct, c, insertLabel ? action.label : null);
 ...
 */
Toolbar.prototype.addButton = function (classname, tooltip, funct, c, insertLabel) {
	const elt = this.createButton(classname, insertLabel);
	c = c != null ? c : this.container;
 ...
 */
Toolbar.prototype.createButton = function (classname, insertLabel) {
	...
	if (insertLabel) {
		mxUtils.write(inner, insertLabel);
	}
 ...
 * Creates and returns a new button.
 */
Toolbar.prototype.createLabel = function (text) {
	const elt = document.createElement('a');
