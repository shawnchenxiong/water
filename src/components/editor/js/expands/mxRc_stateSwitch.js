...
export default mxRc_stateSwitch;

function mxRc_stateSwitch(bounds, fill, stroke, strokewidth) {
    mxShape.call(this), (this.bounds = bounds), (this.fill = fill), (this.stroke = stroke), (this.strokewidth = null != strokewidth ? strokewidth : 1);
};

mxUtils.extend(mxRc_stateSwitch, mxShape);

mxRc_stateSwitch.prototype.cst = {
    SHAPE_NAME: 'mxgraph.rc.mxRc_stateSwitch',
};
mxRc_stateSwitch.prototype.paintVertexShape = function (c, x, y, w, h) {
    this.foreground(c, x, y, w, h);
};
mxRc_stateSwitch.prototype.foreground = function (c, x, y, w, h) {
    if (this.antiAlias) {
        const cell = this.state.cell;
        const graph = this.state.view.graph;
        const style = graph.getCellStyle(cell);
        const imgId = 'img_mxRc_stateSwitch_' + this.state.cell.id;
        const divId = 'div_' + this.state.cell.id;
        cell.setAttribute('divId', divId);
        cell.setAttribute('imgId', imgId);
        let stateImg = mxUtils.fixImg(style.stateValue == '1' ? style.openStateImg : style.closeStateImg);
        var htmlStr = '';
